...
export const synchronize = async (
  ...
  const candidates_update = [...map_prev.values()]
    ...
    .filter(({ prev, next }) => {
      ...
      // biome-ignore lint/suspicious/noExplicitAny: TODO 대충 구현한거
  ...
  if (candidates_insert.length > 0) {
    ...
    const _result = await db.insertInto(table).values(values).execute();
  }
...
export const findByFunctionName = async (db: Kysely<DB>, name: string) => {
